...
\begin{document}
	...
	\chapless{Introducción}
	
	\sectionless{Estructura}
	Cada capítulo contiene una introducción sobre la\textlangle s\textrangle\ corriente\textlangle s\textrangle\ contrincante\textlangle s\textrangle\; los temas de debate con sus respectivas objeciones, respuestas y fuentes; y el caso cumulativo.
	
	Entre los dos primeros capítulos, el primero titulado ``Contra Agnósticos´´ tiene un punto de vista con argumentos filosóficos a favor de un único Dios creador, al margen de una religión concreta; el siguiente titulado ``Contra Ateos´´ contiene objeciones propiamente ateas y argumentos a favor de la Cristiandad. Respecto al Catolicismo como denominación correcta (informalmente)\footnote{Desde la perspectiva católica (formalmente) no existen las denominaciones, tan sólo existe una verdadera Iglesia (``Creo en la Iglesia una, santa, católica y apostólica.´´ $\lfloor$Credo Niceno-Constantinopolitano$\rfloor$).}, será especialmente argumentado en el titulado ``Contra Protestantes´´.
	
	MEJOR: CITAR CORRECTAMENTE CON COMANDOS LATEX

	\chap{Contra Agnósticos}
	
	\sectionless{Sobre Corrientes}
	
	Antes de continuar, es preciso definir los términos que se usarán a continuación. Nota que es posible que el contrincante caiga en uno de estos términos sin él saberlo. Estas son las definiciones más importantes.
	
	\begin{minibloque}[Definiciones]
		\begin{itemize}
			\item \textbf{Agnosticismo:} Cuestiona la existencia de lo divino, sin posicionarse en el lado teísta o ateo. En su versión más formal es la creencia que la existencia de dios\textlangle es\textrangle\ es incognoscible.
			\item \textbf{Teísmo Agnóstico:} Creencia en la existencia de dios\textlangle es\textrangle\ pero afirmando que esta posición es desconocida o incognoscible.
			\item \textbf{Ateísmo Agnóstico:} Creencia en la no existencia de dios\textlangle es\textrangle\ pero afirmando que esta posición es desconocida o incognoscible.
			\item \textbf{Apateísmo:} Apatía sobre el teísmo, la indiferencia ante la cuestión de la existencia de Dios, llevando a un ateísmo práctico. Este término podría incluso aplicar a aquellas personas que se consideran culturalmente religiosas pero no practican la fe.
			\item \textbf{Indiferentismo:} Creencia que considera indiferente adscribirse a una religión u otra.
			\item \textbf{Deísmo:} Creencia en un dios creador que ni interviene ni se revela.
			\item \textbf{Ignosticismo:} Creencia que la cuestión de la existencia de lo divino es absurda, porque la palabra ``Dios´´ carece de sentido o de definición no ambigua.
			\item \textbf{Ietsismo (`algo´-ismo):} Creencia en algo extramaterial o fuerzas sobrenaturales sin adherirse a ningún sistema de creencias. Una mezcla indefinida entre teísmo agnóstico y ateísmo agnóstico.
		\end{itemize}
	\end{minibloque}
	
	Otras definiciones importantes son las posiciones las cuales algunas de estas creencias se someten o aceptan.
	
	\begin{minibloque}[Definiciones]
		\begin{itemize}
			\item \textbf{Relativismo:} Niega la existencia de verdades absolutas, rechaza la unicidad de la verdad, afirmando que esta depende del sujeto o del contexto.
			\item \textbf{Naturalismo:} Afirma que la naturaleza física y sus leyes son lo único existente, negando de manera categórica cualquier realidad o causa sobrenatural.
			\item \textbf{Materialismo:} Reduce toda la realidad a materia y energía; la mente y la moral son meros subproductos físicos. (Todo materialista es naturalista, pero no a la inversa).
			\item \textbf{Cientificismo:} Postura epistemológica que sostiene que el método científico empírico es la única vía válida y fiable para acceder al conocimiento y a la verdad.
			\item \textbf{Determinismo:} Sostiene que toda acción humana está predeterminada por causas físicas previas, eliminando el libre albedrío y la responsabilidad moral.
			\item \textbf{Nihilismo (`nada´-ismo):} Niega la existencia de propósito objetivo, valores morales intrínsecos o significado último en la existencia.
			\item \textbf{Postmodernismo:} Rechaza los grandes relatos y metanarrativas (como la religión), fragmentando la realidad en interpretaciones e ideas individuales.
			\item \textbf{Posteísmo:} Sostiene que el debate sobre la existencia de Dios ha quedado obsoleto y superado por el progreso humano.
		\end{itemize}
	\end{minibloque}
	
	https://www.gotquestions.org/atheist-Christian.html\\
	https://www.gotquestions.org/ignosticism.html\\
	https://www.gotquestions.org/post-theism.html\\
	https://en.wikipedia.org/wiki/Apatheism
	
	\section{Apateísmo}
	
	\subsubsection{No Neutralidad del Apateísmo}
	
	\begin{minibloque}[Objeción]
		Mi posición es totalmente neutra: no afirmo ni niego la existencia de Dios, simplemente prefiero no tomar ninguna postura al respecto.
	\end{minibloque}
	\begin{minibloque}[Respuesta]
		\begin{itemize}
			\item \textbf{Imposibilidad de la Neutralidad:} En la vida práctica no existe la pausa existencial. Ignorar la existencia de Dios equivale en la conducta diaria a actuar como si Dios no existiera (ateísmo práctico/agnóstico).
			\item \textbf{Elección por Omisión:} La apatía no deja la cuestión en suspenso, asume de forma implícita el resultado del ateísmo sin haber verificado sus premisas ni haber evaluado sus consecuencias.
			\item \textbf{Fe Atea:} Ser un ateo práctico apático conlleva una fe ciega grandísima, una posición atea o teísta argumentada/evidencializada siempre requerirá de una fe razonable.
		\end{itemize}
	\end{minibloque}
	
	\subsubsection{Evidnecias y Relevancia de la Cuestión Teísta}
	
	\begin{minibloque}[Objeción]
		La cuestión teísta es poco relevante por su falta de evidencia (como los extraterrestres). Hay infinitas afirmaciones metafísicas, no tenemos el deber de investigarlo todo.
	\end{minibloque}
	\begin{minibloque}[Respuesta]
		\begin{enumerate}
			\item \textbf{Es Relevante:} Conocer la existencia de un ser supremo es de máxima relevancia si este se ha revelado y ha intervenido en la historia para dar mandamientos e implicaciones eternas.
			\item \textbf{La Apuesta de Pascal:} Dada la posible existencia de un Dios omnipotente y justo, es matemáticamente lógico buscarle y obedecerle: si existe y se le obedece, la recompensa es $\infty$; si no existe, la pérdida es $0$; pero si existe y se le ignora o desobedece, el resultado es $-\infty$.
			\item \textbf{Existencia de Evidencias:} Existen evidencias contundentes a nivel filosófico sobre la Causa Primera, así como pruebas históricas y arqueológicas sobre su Revelación. (CITAR CONTRA ATEOS Y PROTESTNATES LOS MILAGROS especialmente (evidencia con implicaciones directas))
			\item \textbf{Importancia de las Evidencias:} Al existir evidencias robustas que otorgan una probabilidad real a su existencia ($P > 0$), la apuesta de Pascal cobra plena validez y vuelve la cuestión supremamente urgente.
		\end{enumerate}
	\end{minibloque}

	La apatía es querer permanecer en ignorancia voluntariamente, cosa que es cobarde y malo, pues el saber es bueno.
	
	
	\subsubsection{¿Cúal de Tantos Dioses?, ¿Por Qué el Cristianismo?}
	
	\begin{minibloque}[Objeción]
		Han existido miles de religiones y divinidades. Es imposible investigarlas todas para saber cuál es la correcta, por lo que elegir una resulta arbitrario y la búsqueda se vuelve inútil.
	\end{minibloque}
	
	\begin{minibloque}[Respuesta]
		\begin{enumerate}
			\item \textbf{Filtro Politeísta:} Con <<Dios>> hablamos del ser creador supremo, este debe tener una unicidad, pues no pueden existir varios dioses supremos ni diversas <<Primeras Causas>>.
			\item \textbf{Filtro Histórico de la Revelación:} Si Dios se ha revelado, su mensaje debe contar con registros históricos, continuidad doctrinal y testimonios verificables.
			\item \textbf{El Caso Cristiano:} Es la opción intrínsecamente más probable entre las confesiones teístas.
			\begin{itemize}
				\item Tiene inicios temporales y geográficos claros, no cae en la narrativa de `hace mucho tiempo en un lugar muy lejano...´.
				\item En los principios de la Revelación (1900–1800 $\tilde{\text{a}}$C), era rarísima una religión que creía en un solo Dios y un principio del universo (ahora respaldado por el Big Bang).
				\item Continuidad revelada a lo largo de siglos mediante profecías mesiánicas progresivas cumplidas y datos históricos coherentes.
				\item La proposición más atrevida y radical sobre cualquier otra religión: Jesús encarnado es Dios.
				\item Los apóstoles no murieron por una fe abstracta, sino como testigos oculares de la Resurrección. No tantos (o nadie) se sacrifican conscientemente por defender una mentira que saben que es falsa. Esto contrasta con los mártires de otras religiones, que mueren por lo que creen verdadero.
				\item La doctrina tiene estándares morales fuertes, y una visión del perdón radical y única, pero a su vez comprensible desde su benevolencia.
			\end{itemize}
		\end{enumerate}
	\end{minibloque}
	...
	%https://www.reddit.com/r/DebateReligion/comments/35q6au/can_we_talk_about_apatheism/?tl=es-es
		
	Es una fe al <<no>> de la existencia de Dios
	
	Pensamientos (Pensées), Blaise Pascal (especialmente el fragmento sobre los indiferentes).
	
	Trevor Treharne (How to be a Happy Atheist) o Jonathan Rauch.
	Cristianismo... y nada más (Libro 1: "El bien y el mal como clave para comprender el universo").
	
	Autor: Friedrich Nietzsche.\\
	Obras: La gaya ciencia (§125, "El loco") y La genealogía de la moral.\\
	Por qué sirve: Nietzsche es el mejor aliado del apologista contra el agnóstico cómodo. Él vio claramente que si quitas a Dios, no puedes conservar la moral cristiana de "amar al prójimo" ni los derechos humanos. Citar a Nietzsche demuestra a tus amigos que el propio ateísmo radical reconoce que sin Dios todo el sistema de valores mundano se desmorona.
	
	apuesta de pascal modificada al estudio tras mostrar evidencias (pues la apuesta original sólo tiene un cuenta una religión)
	
	\begin{minibloque}[Fuentes]
		https://www.gotquestions.org/apatheism.html
	\end{minibloque}
	
	\section{Relativismo}
	Absurdo, mismamente afirma su propia postura absolutamente, anti-cientifico, moral relativa tiene probelmas graves (como que por algunas personas puede ser que torturas menores sea moralmente positivo). Es la posición más comoda junto el apateísmo, negarse a pensar más alla, ``todo es verdad, no hace falta buscar más´´.
	
	\section{Relativismo Moderado}
	Esta es la corrección del anterior, pero gana en arbitrariedad, el relativismo ahora se limita a la moralidad o a otras cuestiones menores, tolerando la ciencia.
	
	\section{Fe en la absencia de lo divino}
	
	
	\section{¿Por qué el Cristianismo Católico?}
	
	
	\sectionless{Caso Cumulativo}



	\chap{Contra Ateos}
	
	\section{Veracidad de la Biblia}
	\subsection{Los textos conservados más antiguos datan después de Cristo}
	...
	\begin{boxless1}
		Este capítulo se centrará prioritariamente en el Islam suní por representar la corriente hegemónica del Islam. Refutar el sunismo implica desmontar la columna vertebral teológica y jurídica de la gran mayoría del mundo islámico.
	\end{boxless1}
	...
	Aceptan la legitimidad de los primeros 4 sucesores de Mahoma (Abu Bakr, Úmar ibn al-Jattab, Uthmán ibn Affán, Alí ibn Abi Tálib). De acuerdo con las tradiciones sunitas, Mahoma no designó claramente un sucesor y la comunidad musulmana actuó de acuerdo con su Sunna eligiendo a su suegro Abu Bakr como primer califa.
	
	\subsubsection{Islam Chií}
	\varfootnote{Es llamado chiismo la rama chií o chiita o chia, los seguidores son chiíes o chiitas.}	
	Cerca del 10\% de los musulmanes son chiíes y se concentran especialmente en Azerbaiyán, Irak e Irán. Siguen los preceptos de hadices diferentes a los de los suníes y tienen sus propias tradiciones legales. Los eruditos chiíes tienen mayor autoridad que los suníes y mayor amplitud para la interpretación del Corán y de los hadices.
	
	No aceptan la legitimidad de los primeros 3 califas suníes, sino que sostienen que Mahoma anunció a su yerno y primo Alí ibn Abi Tálib como su sucesor, particularmente durante el sermón de Ghadir Khumm. Tras la muerte Uthmán ibn Affán, Alí es finalmente elegido califa.
	
	\pagebreak % CUTRADA
	\subsubsection{Diferencias}
	
	\begin{tab}{3}{|+f|-C|-C|}{Tema & Suní & Chií}
		Infalibilidad & Sólo los profetas & Los profetas y los Imames\footnote{Los 12 sucesores chiíes de Mahoma. Para los suníes un Imam (o Imán) es quien dirige el discurso en una mezquita.} \\\hline
	...
	En el Islam, la fe tiene 6 pilares: La creencia en Allah, sus ángeles, sus libros sagrados, sus Profetas y Mensajeros, el Día del Juicio, y el Destino, que son credo de la fe islámica.
	
	\begin{itemize}
		\item \textbf{Tawhid (Unicidad):} Creencia en Allah como Dios único, indivisible y absoluto.
		\item \textbf{Ángeles:} Seres creados de luz que ejecutan las órdenes de Allah (destacando a Gabriel (\textit{Jibril})).
	...
	\subsubsection{Clasificación de los Hadices}
	
	\begin{itemize}
		\item \textbf{Qudsi} \textipa{[qud.si:]}: Sagrados, portadores de la palabra divina y no de la de Mahoma.
		\item \textbf{Mutawatir} \textipa{[mu.ta.wa:.tiR]}: Recurrentes.
		\item \textbf{Sahih (Grado)} \textipa{[sa\textcrh.i:\textcrh]}: Auténticos.
		\item \textbf{Hasan (Grado)} \textipa{[\textcrh a.san]}: Buenos.
		\item \textbf{Da'if (Grado)} \textipa{[da\textrevglotstop.i:f]}: Débiles, normalmente apócrifos.
	\end{itemize}

	\subsection{Términos Clave}
	
	\begin{itemize}
		\item \textbf{Tawhid} \textipa{[taw.\textcrh i:d]} (\textarabic{توحيد}): La unicidad absoluta de Allah.
	...
	\begin{bloque}[Evidencia en las Fuentes Primarias Suníes]
		...
		\begin{enumerate}
			...
			<<El Profeta (\textarabic{صَلَّى اللّٰهُ عَلَيْهِ وَسَلَّمَ}) se casó conmigo cuando yo era una niña de seis años, y consumó el matrimonio conmigo cuando yo tenía nueve años>>\footnote{Texto en árabe: \textarabic{تَزَوَّجَنِي النَّبِيُّ وَأَنَا بِنْتُ سِتِّ سِنِينَ، وَبَنَى بِي وَأَنَا بِنْتُ تِسْعِ سِنِينَ}. La expresión \textit{banā bī} (\textarabic{بَنَى بِي}) es el eufemismo clásico para la consumación del matrimonio.}.
			...
			\item \textbf{El Marco Jurídico del Corán [65.4]:} Al legislar el periodo de espera (\textarabic{عِدَّة}, \textit{'iddah}) para el divorcio, el Corán incluye explícitamente a las mujeres que aún no han menstruado (\textarabic{وَاللَّائِي لَمْ يَحِضْنَ}, \textit{wa al-la'i lam yahidna}), validando jurídicamente el matrimonio y consumación con prepúberes en la jurisprudencia (\textarabic{فِقْه}, \textit{fiqh}).
		\end{enumerate}
	\end{bloque}
	...
	\begin{minibloque}[Respuesta]\leavevmode
		\begin{enumerate}
			...
			\[
			\text{YHWH} + \text{Vocales}(\text{Adonái}) \mapsto \text{YeHoWaH }(\mhebrew{יְהֹוָה},\textrm{ Jehová en español})
			\]
